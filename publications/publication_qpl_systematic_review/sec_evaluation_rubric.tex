...
The instrument operationalises the taxonomy from Section~\ref{sec:taxonomy} and the quantum-implicit gating constraints in Subsection~\ref{subsec:evaluation_instrument:gating} are the operational form of the quantum-implicit litmus test stated in that section.
...
\begin{description}
    \item[$T_1$ Deutsch--Jozsa] Implement the Deutsch--Jozsa decision procedure for a small input size, including an explicit oracle definition or the system's intended oracle hook.
    ...
    If the system requires measurement or sampling to get the decision, the observation boundary is recorded under the conditions in~\ref{subsec:evaluation_instrument:conditions}.
    ...
    The minimum deliverable defines an oracle that marks a single basis state or simple predicate, implements one full Grover iteration as an oracle phase flip followed by diffusion and provides the canonical pathway to execute and get an outcome.
    Any explicit sampling, shots or observable boundary is recorded under the conditions in~\ref{subsec:evaluation_instrument:conditions}.
    ...
    If measurement is required, the observation boundary is recorded under the conditions in~\ref{subsec:evaluation_instrument:conditions}.
    ...
    Measurement is avoided in the core construction where possible; if it is required by the system's result model, the boundary is recorded under the conditions in~\ref{subsec:evaluation_instrument:conditions}.
\end{description}
...
\begin{table}[ht*]
    ...
    \begin{tabular}{p{1.2cm}ccccccccc}
        ...
        Framework        & $F$  & $F$          & $F$          & $F$  & $C$/$F$ & $F$          & $P$  & $C$/$F$ & $C$          \\ \addlinespace
        High-level       & $C$  & $C$          & $C$          & $C$  & $C$     & $C$          & $P$  & $C$     & $P$          \\ \addlinespace
