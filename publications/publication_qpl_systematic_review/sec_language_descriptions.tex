\section{Language descriptions}\label{sec:pub_qpl_review:language_descriptions}
The eleven systems surveyed were selected according to criteria C\textsubscript{1}--C\textsubscript{4} defined in Section~\ref{sec:pub_qpl_review:taxonomy} and the inclusion/exclusion protocol described in Section~\ref{sec:pub_qpl_review:review_methodology}.
Systems are described in implied abstraction tier order: Assembly, Intermediate Representation, Embedded Circuit Framework and High-Level Language, mirroring the taxonomy structure from Section~\ref{sec:pub_qpl_review:taxonomy}.
No tier assignments, rubric scores or evaluative language appear in this section; all evaluation is deferred to Section~\ref{sec:pub_qpl_review:language_assessments}.
Quipper is described in Subsection~\ref{subsec:pub_qpl_review:language_descriptions:quipper} but does not satisfy inclusion criteria C\textsubscript{2} and C\textsubscript{3}; it is retained as a comparison case for the non-Python embedded DSL tradition and excluded from primary assessment in Section~\ref{sec:pub_qpl_review:language_assessments}.

\subsection{OpenQASM 3.0}\label{subsec:pub_qpl_review:language_descriptions:openqasm}
...
\subsection{QIR}\label{subsec:pub_qpl_review:language_descriptions:qir}
...
\subsection{Q-MLIR}\label{subsec:pub_qpl_review:language_descriptions:qmlir}
...
\subsection{Qiskit}\label{subsec:pub_qpl_review:language_descriptions:qiskit}
...
Qiskit is the most widely adopted quantum programming framework by ecosystem breadth and project count~\cite{murillo2022sequantumprogramming,corralesgarro2025productivityexpressiveness}, providing the central reference point for the embedded circuit framework tier in this review.

\subsection{PennyLane}\label{subsec:pub_qpl_review:language_descriptions:pennylane}
...
\subsection{Quipper}\label{subsec:pub_qpl_review:language_descriptions:quipper}
\textit{Quipper} is a functional quantum programming language embedded in Haskell, designed to generate and manipulate quantum circuits as first-class values~\cite{green2013quipper}.
The language uses monadic operational semantics, encapsulating qubit operations within the \texttt{Circ} monad.
Circuits are constructed by composing gate applications, higher-order combinators and named circuit blocks defined with the \texttt{box} construct.
Quipper compiles to parameterised object code that generates circuits at runtime rather than emitting static circuit descriptions, enabling circuits of practically large scale to be expressed concisely.
Dynamic lifting permits measurement results from one circuit to become parameters of a subsequent circuit, supporting multi-stage classical-quantum workflows~\cite{green2013quipper}.
Quipper does not satisfy inclusion criteria C\textsubscript{2} and C\textsubscript{3} (Sections~\ref{sec:pub_qpl_review:taxonomy}and~\ref{sec:pub_qpl_review:review_methodology}) and is excluded from primary assessment in Section~\ref{sec:pub_qpl_review:language_assessments}.

\subsection{Q\#}\label{subsec:pub_qpl_review:language_descriptions:qsharp}
...
\subsection{Silq}\label{subsec:pub_qpl_review:language_descriptions:silq}
...
\subsection{Qmod}\label{subsec:pub_qpl_review:language_descriptions:qmod}
...
\subsection{Quff}\label{subsec:pub_qpl_review:language_descriptions:quff}
...
\subsection{Qutes}\label{subsec:pub_qpl_review:language_descriptions:qutes}
...
The most notable abstraction is a Grover-based substring search operator: a quantum string containment test expressed as an \texttt{in} predicate, with the amplitude-amplification procedure entirely hidden from the programmer.
