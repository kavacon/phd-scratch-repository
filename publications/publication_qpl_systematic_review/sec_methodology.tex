...
The unit of analysis for extraction and scoring is each included system’s authoritative evidence base (primary paper/specification and official documentation) rather than the full retrieved corpus.
...
\begin{description}
    ...
    \item[$C_4$] Public documentation is sufficient to complete tasks defined in Section~\ref{subsec:evaluation_instrument:task_suite} without access to private knowledge.
\end{description}
...
Per-system outputs follow the aggregation rules defined in Section~\ref{sec:evaluation_instrument}: headline score, spectrum score (arithmetic mean of A--E with minimum--maximum range) and quantum bias count.
...
The HPDC~2025 paper alone~\cite{faro2025qutes} would not sustain either score; removal would reduce both to score~2, shifting the headline from high-level to framework.
