...
The spectrum is organised around two dimensions of progressive abstraction: the degree to which quantum mechanical concerns must be visible at the programming surface, and the degree to which quantum realisation is a compiler responsibility rather than a programmer responsibility.
These dimensions follow directly from the abstraction hierarchy established in prior work~\cite{DiMatteo2024AbstractionHierarchy} and operationalised in the bleed-through concept introduced in Section~\ref{sec:introduction}.
...
\begin{description}
    ...
    \item[Framework]
    Framework-tier systems are libraries or SDKs that may be hosted inside a general-purpose language like Python.
    ...
    The key boundary from the high-level tier is that frameworks expose the circuit as the primary artefact; high-level systems treat circuit construction as an implementation detail managed by the compiler.
...
Tier assignments are not reported here; they are findings produced by applying the evaluation instrument defined in Section~\ref{sec:evaluation_instrument} and are reported in Section~\ref{sec:system_assessments}.
...
\begin{table}[htbp*]
    ...
    \begin{threeparttable}
        ...
        \begin{tabular}{ll}
            ...
            \addlinespace
            Quipper~\cite{green2013quipper}\tnote{\dag}               & Embedded circuit framework   \\
            \addlinespace
            Tower~\cite{yuan2025foundationalabstractions}\tnote{\dag} & Embedded circuit framework   \\
            \bottomrule
        \end{tabular}
        \begin{tablenotes}
            \footnotesize
            \item[\dag] Unassessed and provided for comparison only; see Section~\ref{subsec:methodology:quality}.
        \end{tablenotes}
    \end{threeparttable}
\end{table}
