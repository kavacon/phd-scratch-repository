...
\begin{longtable}{p{1.8cm} p{3.3cm} p{3.3cm} p{3.3cm} p{3.3cm}}
    ...
    & \textit{No circuit elements required to reason about program effects on state.}
    & \textit{No reference to circuit elements in data types.}
    & \textit{Classical and quantum instructions coexist at the same level.}
    & \textit{Shot count elided; distribution types well-defined at the language level.} \\     \midrule
...
A survey of 251 active quantum programmers found that 83\% hold at least five years of classical programming experience, yet quantum vocabulary remains the primary-reported barrier to productive use across all frameworks and languages~\cite{ferreira2024qplusage}.
Violations are consistent across advanced quantum programming systems, including the system that advances furthest toward data structure abstraction operating in superposition, confirming that this ceiling is a structural property of the existing design space.
...
It is not possible to lift classical abstractions into superposition and form a complete quantum language, new paradigms and new protocols are required.
...
Achieving that separation requires a language whose abstraction model eliminates bleed-through between the programming layer and the execution and hardware layers.
Its surface model must be representation-independent and its conceptual foundations must be designed around quantum-mechanical constraints rather than classical abstractions adapted to accommodate them.
The No-Embedding Theorem establishes that this constraint is structural rather than contingent~\cite{yuan2025foundationalabstractions}.
Classical abstraction models and attempts to extend them to quantum computation cannot resolve it.
